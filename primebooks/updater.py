# primebooks/updater.py
"""
PrimeBooks Auto-Update System
✅ Checks for updates from server
✅ Notifies users of available updates
✅ Downloads updates in background
✅ Prompts for installation (Update Now / Remind Later)
✅ Critical updates with countdown timer
...
CURRENT_VERSION = "1.0.0"  # ✅ Update this when releasing new version
...
class UpdateDownloader(QThread):
    ...
    def __init__(self, download_url, auth_token):
        ...
        self.auth_token = auth_token
        self.download_path = settings.DESKTOP_DATA_DIR / 'updates' / 'PrimeBooks_Update.exe'
...
class UpdateAvailableDialog(QDialog):
    ...
    ✅ Shows version, release notes, file size
...
class CriticalUpdateDialog(QDialog):
    """
    Dialog for critical/mandatory updates
    ✅ No "Remind Later" option
    ✅ Countdown timer
    ✅ Shows scheduled maintenance time
    """
    ...
    def __init__(self, parent, update_info):
        ...
        self.setWindowTitle("⚠️ Critical Update Required")
        self.setFixedWidth(500)
        self.setModal(True)

        # Get maintenance time
        self.maintenance_time = datetime.fromisoformat(
            update_info.get('maintenance_start', datetime.now().isoformat())
        )
    ...
    def setup_ui(self):
        ...
        title = QLabel("<h2>⚠️ Critical Update Required</h2>")
        title.setStyleSheet("color: #e74c3c;")
        layout.addWidget(title)

        # Message
        message = QLabel(
            "<b>A critical update is required for PrimeBooks.</b><br><br>"
        ...
        maintenance_msg = QLabel(
            f"<b>Scheduled Maintenance:</b><br>"
            f"{self.maintenance_time.strftime('%B %d, %Y at %I:%M %p')}<br><br>"
            f"<b>Estimated Duration:</b> 1 hour"
        )
        maintenance_msg.setStyleSheet("padding: 10px; background: #fff3cd; border-radius: 4px;")
        layout.addWidget(maintenance_msg)

        # Countdown label
        self.countdown_label = QLabel("")
        self.countdown_label.setStyleSheet("font-size: 16px; font-weight: bold; color: #e74c3c;")
        layout.addWidget(self.countdown_label)

        # Instructions
        instructions = QLabel(
            "⚠️ Please save your work. The application will need to restart to apply the update."
        )
        instructions.setWordWrap(True)
        instructions.setStyleSheet("color: #e67e22;")
    ...
    def update_countdown(self):
        ...
        now = datetime.now()
        time_until = self.maintenance_time - now

        if time_until.total_seconds() <= 0:
            self.countdown_label.setText("⏰ Maintenance starting now!")
            self.timer.stop()
            ...
            self.countdown_label.setText(
                f"⏰ Time until maintenance: {hours:02d}:{minutes:02d}:{seconds:02d}"
            )
...
class UpdateManager:
    ...
    def on_update_available(self, update_info):
        ...
        if is_critical:
            # Show critical update dialog with countdown
            dialog = CriticalUpdateDialog(self.main_window, update_info)
