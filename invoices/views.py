...
def invoice_print_view(request, pk):
    """Generate a printable invoice view for the current tenant"""

    # ✅ Always operate within the current tenant schema
    tenant = get_tenant(request)

    # Get the invoice only from this tenant’s schema
    invoice = get_object_or_404(Invoice, pk=pk)

    # ✅ Get the tenant’s company info (usually one per tenant)
    try:
        company = Company.objects.get(schema_name=tenant.schema_name)
    except Company.DoesNotExist:
        company = None

    # ✅ Select the active invoice template for this tenant
    template = (
            InvoiceTemplate.objects.filter(is_default=True).first()
            or InvoiceTemplate.objects.first()
    )
    ...
    context = {
        ...
        'template': template,
        'company_info': {
            'name': company.name if company else tenant.name,
...
def invoice_dashboard(request):
    """Advanced dashboard with comprehensive metrics and analytics"""
    ...
    invoices_this_month = invoices.filter(
        issue_date__gte=today.replace(day=1)
    ).count()
    ...
    recent_invoices = Invoice.objects.select_related(
        'sale__customer', 'created_by', 'store'
...
def api_revenue_trend(request):
    ...
    # Apply filters
    invoices = Invoice.objects.filter(issue_date__range=[start_date, today])
    ...
    if period == 'daily':
        for i in range(days):
            ...
            day_invoices = invoices.filter(issue_date=date)

            labels.append(date.strftime('%b %d'))
            revenue_data.append(
                float(day_invoices.filter(status='PAID').aggregate(
                    Sum('total_amount'))['total_amount__sum'] or 0)
            )
            count_data.append(day_invoices.count())
        ...
        for i in range(weeks):
            ...
            revenue_data.append(
                float(week_invoices.filter(status='PAID').aggregate(
                    Sum('total_amount'))['total_amount__sum'] or 0)
            )
            count_data.append(week_invoices.count())
        ...
        for i in range(12):
            ...
            if month_date.month == 12:
                month_end = month_date.replace(month=12, day=31)
            else:
            ...
            revenue_data.insert(
                0,
                float(month_invoices.filter(status='PAID').aggregate(
                    Sum('total_amount'))['total_amount__sum'] or 0)
            )
            count_data.insert(0, month_invoices.count())
...
def api_payment_methods(request):
    """API endpoint for payment methods distribution"""
    from django.db.models import Sum, Count
    ...
    for method in payment_methods:
        labels.append(method['payment_method'])
        amounts.append(float(method['total_amount']))
...
def api_top_customers(request):
    """API endpoint for top customers by revenue"""
    from django.db.models import Sum, Count
    ...
    invoices = Invoice.objects.filter(sale__customer__isnull=False)
    ...
    for customer in top_customers:
        customers.append(customer['sale__customer__name'][:20])  # Truncate long names
        revenue.append(float(customer['total_revenue']))
...
@login_required
@permission_required('invoices.view_invoice')
def api_efris_status(request):
    """API endpoint for EFRIS integration status"""
    from django.utils import timezone
    from datetime import timedelta
    ...
    # Failed today
    failed_today = Invoice.objects.filter(
        fiscalization_status='failed',
        fiscalization_time__date=today
    ).count()
    ...
    fiscalized = Invoice.objects.filter(is_fiscalized=True).count()

    fiscalization_rate = (fiscalized / total_invoices * 100) if total_invoices > 0 else 0

    return JsonResponse({
        'pending_fiscalization': pending,
        'failed_today': failed_today,
        'fiscalization_rate': fiscalization_rate,
        'total_invoices': total_invoices,
        'fiscalized_invoices': fiscalized
    })
...
def api_alerts(request):
    ...
    if overdue_count > 0:
        alerts.append({
            ...
            'message': 'You have invoices that are past their due date and require immediate attention.',
            'action': 'View Overdue',
            'action_url': '/invoices/?status=OVERDUE'
        })

    # Check for pending fiscalization
    pending_fiscalization = Invoice.objects.filter(
        is_fiscalized=False,
        status__in=['SENT', 'PAID'],
        issue_date__gte=today - timedelta(days=7)
    ).count()

    if pending_fiscalization > 5:
        alerts.append({
            'type': 'warning',
            'icon': 'shield-alt',
            'title': f'{pending_fiscalization} Invoices Pending Fiscalization',
            'message': 'Multiple invoices are awaiting EFRIS fiscalization. Consider bulk fiscalization.',
            'action': 'Fiscalize Now',
            'action_url': '/invoices/bulk-fiscalize/'
        })

    # Check for large pending payments
    large_pending = Invoice.objects.filter(
        status__in=['SENT', 'PARTIALLY_PAID']
    ).annotate(
    ...
    if large_pending > 1000000:  # UGX 1M
        alerts.append({
            'type': 'warning',
            'icon': 'money-bill-wave',
            'title': 'Large Outstanding Amount',
            'message': f'UGX {large_pending:,.0f} in outstanding payments. Follow up with customers.',
            'action': 'View Pending',
            'action_url': '/invoices/?status=SENT'
        })

    # Check for failed fiscalizations
    failed_today = Invoice.objects.filter(
        fiscalization_status='failed',
        fiscalization_time__date=today
    ).count()
    ...
    if failed_today > 0:
        alerts.append({
            'type': 'danger',
            'icon': 'times-circle',
            'title': f'{failed_today} Failed Fiscalizations Today',
            'message': 'Some invoices failed EFRIS fiscalization. Check the audit log for details.',
            'action': 'View Audit',
            'action_url': '/invoices/fiscalization-audit/'
        })
...
def api_metrics(request):
    """API endpoint for updated metrics based on filters"""
    from django.db.models import Sum, Count
    from django.utils import timezone
    ...
    total_revenue = invoices.filter(status='PAID').aggregate(
        Sum('total_amount'))['total_amount__sum'] or 0
    pending_invoices = invoices.exclude(
        status__in=['PAID', 'CANCELLED']).count()
    overdue_invoices = invoices.filter(
        due_date__lt=timezone.now().date(),
        status__in=['SENT', 'PARTIALLY_PAID']
    ).count()
