...
def _get_company_from_request(request):
    """Get tenant company — matches your existing codebase pattern."""
    # Try request.tenant first (set by TenantMainMiddleware)
    company = getattr(request, 'tenant', None)
    if company:
        return company

    # Fallback: get from the database connection (django-tenants)
    from django.db import connection
    ...
    if schema and schema != get_public_schema_name():
        try:
            ...
            return Company.objects.get(schema_name=schema)
        except Exception:
            pass

    # Final fallback: same as your invoices/views.py get_current_tenant()
    return getattr(request.user, 'company', None) if hasattr(request, 'user') else None
...
def _get_or_register_tenant_ipn(request, company) -> str | None:
    ...
    TOKEN CACHE FIX:
    Previously, calling PesapalService.for_tenant() here AND again inside
    svc.submit_order() each triggered a fresh /Auth/RequestToken round-trip
    (~1 s each). The fix caches the already-registered IPN id on the
    TenantPesapalConfig row so that subsequent calls within the same flow
    (and across requests) skip the GetIpnList + RegisterIPN leg entirely,
    leaving only a single token fetch before SubmitOrderRequest.
    """
    # Fast path: IPN id already persisted — skip all Pesapal calls.
    try:
    ...
    if result['success']:
        # Persist so the next request skips registration entirely.
        try:
            if cfg and not cfg.ipn_id:
...
def initiate_pesapal_payment(request, sale_id: int):
    ...
    # Validate — only reject if genuinely already PAID (not just PENDING)
    if sale.payment_status == 'PAID':
        return JsonResponse(
            {'success': False, 'error': f'Sale {sale.document_number} is already paid'},
            status=400
        )
