...
def company_action(request, company_id):
    """Perform SaaS admin action on a company"""
    ...
    return redirect("system_companies_list")
    ...
    def _get_company_statistics_with_analytics(self, company):
        ...
        try:
            ...
            stats['cashiers'] = all_employees.filter(
                user_type='CASHIER', is_active=True
            ).count()
    ...
    def _get_management_analytics(self, company):
        ...
        try:
            ...
            try:
                ...
                ).select_related('user', 'device__store__branch').order_by('-timestamp')[:15]
            ...
            staff_metrics = {
                ...
                'staff_by_role': {
                    ...
                    'managers': all_staff.filter(user_type='MANAGER').count(),
                    'cashiers': all_staff.filter(user_type='CASHIER').count(),
                    'employees': all_staff.filter(user_type='EMPLOYEE').count(),
                },
...
class CompanyUpdateView(LoginRequiredMixin, PermissionRequiredMixin, CompanyFormMixin, UpdateView):
    """Enhanced company update view with formset support and proper access control."""
    model = Company
    form_class = CompanyForm
    template_name = 'company/company_form.html'
    ...
    def dispatch(self, request, *args, **kwargs):
        logger.debug(f"CompanyUpdateView accessed by user={request.user}, kwargs={kwargs}")
        try:
            return super().dispatch(request, *args, **kwargs)
        except Exception as e:
            logger.error(f"Error in dispatch for CompanyUpdateView: {e}", exc_info=True)
            raise

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        logger.debug(f"Building context data for company={self.object}")

        context.update({
            'form_title': _('Update Company'),
            'submit_text': _('Update Company'),
            'is_update': True,
        })
        ...
        if self.object:
            try:
                total_branches = Store.objects.filter(company=self.object).count()
                active_branches = Store.objects.filter(company=self.object, is_active=True).count()
                total_employees = CustomUser.objects.filter(company=self.object, is_hidden=False).count()
                active_employees = CustomUser.objects.filter(
                    company=self.object, is_active=True, is_hidden=False
                ).count()
                ...
                logger.debug(
                    f"Company stats for {self.object.company_id}: "
                    f"branches={total_branches}/{active_branches}, "
                    f"employees={total_employees}/{active_employees}"
                )
            except Exception as e:
                logger.error(f"Error building context for company {self.object}: {e}", exc_info=True)
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            logger.error(f"Error in form_valid for company {self.object}: {e}", exc_info=True)
            raise
    ...
    def get_success_url(self):
        ...
        url = reverse('companies:company_detail', kwargs={'company_id': self.object.company_id})
        logger.debug(f"Redirecting to success URL: {url}")
        return url
...
class EmployeeCreateView(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    ...
    fields = ['username', 'email', 'first_name', 'last_name', 'phone_number',
              'user_type', 'company_admin', 'is_active']
...
def company_expired_view(request):
    ...
    if company.is_trial and company.trial_ends_at:
        days_expired = (timezone.now().date() - company.trial_ends_at).days
    elif company.subscription_ends_at:
        days_expired = (timezone.now().date() - company.subscription_ends_at).days
    else:
        days_expired = 0
