...
def handle_expense_status_change(sender, instance, created, **kwargs):
    ...
    if not created:
        # Check if status changed without using tracker
        try:
            old_instance = Expense.objects.get(pk=instance.pk)
            if old_instance.status != instance.status:
                send_expense_status_notification(instance, old_instance.status)
        except Expense.DoesNotExist:
            pass  # Instance doesn't exist anymore
...
def send_expense_status_notification(expense, old_status):
